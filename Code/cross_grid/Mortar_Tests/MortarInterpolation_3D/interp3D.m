...
for i = 0:nSizes-1
    ...
    [E_RBF_g,~,~] = mortar.computeMortarRBF(nG,nInt,'gauss');
    [E_RBF_w,M1,t1] = mortar.computeMortarRBF(nG,nInt,'wendland');
    %[E2,M2,t2] = mortar.computeMortarElementBased(nG);
...
loglog(h,L2_rbf_w,'b-^')
loglog(h,L2_eb,'g-^')
legend('RBF - Rescaled Gauss', 'RBF - Rescaled Wendland')
xlabel('mesh size')
ylabel('Quadratic error of interpolation')

% plotFunction(msh1, 'out_master', fIn)
% plotFunction(msh2, 'out_slaveRBF', fOutRBF)
