...
for integration = int_str
    if strcmp(integration,'RBF')
        for nG = np
        fopen(strcat(tmp,'nInt',num2str(nG),'L2_rbf.txt'),'w');
        fopen(strcat(tmp,'nInt',num2str(nG),'H1_rbf.txt'),'w');
        end
    ...
    for mCount = 1:nGrids
        for nG = np
            ...
            if strcmp(integration,'RBF')
                [E, M, D] = compute_mortar(masterMesh, slaveMesh, [], nG, 5, 1, 1, "RBF");
            elseif strcmp(integration, 'SB')
            ...
            if strcmp(integration,'RBF')
                fID1 = fopen(strcat(tmp,'nInt',num2str(nG),'L2_rbf.txt'), 'a');
                fID2 = fopen(strcat(tmp,'nInt',num2str(nG),'H1_rbf.txt'), 'a');
            elseif strcmp(integration, 'SB')
...
if any(strcmp(int_str, 'RBF'))
    for nG = np
        fName = strcat(tmp,'nInt',num2str(nG),'H1_rbf.txt');
        H1_RBF = load(fName);
        loglog(H1_RBF(:,1), H1_RBF(:,2), '--ro', 'LineWidth', 1, 'MarkerSize', 8.5)
        hold on
        leg_str = [leg_str, strcat("RBF integration nGP = ",num2str(nG))];
    end
