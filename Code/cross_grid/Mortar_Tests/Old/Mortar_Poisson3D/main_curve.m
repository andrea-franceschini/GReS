clear
close all
% Mortar solution of a simple Poisson problem on a 3D domain with curved
% interface
...
elseif strcmp(flagLeft, 'slave')
    fileNameSlave = 'meshCurve/LeftBlock_curve.msh';
    fileNameMaster = 'meshCurve/RightBlock_curve.msh';
    strLeft = 'slaveLeft';
end
...
if fPlot ==  true
    dir = 'curve';
    fNameMaster = strcat(dir,strLeft,'_SolMaster','_',type);
    fNameSlave = strcat(dir,strLeft,'_SolSlave','_',type);
    plotParaview(masterMesh,fNameMaster, u_master', 'x')
    plotParaview(slaveMesh,fNameSlave, u_slave', 'x')
    fNameMaster = strcat(dir,strLeft,'_errRelMaster','_',type);
    fNameSlave = strcat(dir,strLeft,'_errRelSlave','_',type);
    plotParaview(masterMesh,fNameMaster, err_rel_master', 'x')
    plotParaview(slaveMesh,fNameSlave, err_rel_slave', 'x')
    fNameMaster = strcat(dir,strLeft,'_errAbsMaster','_',type);
    fNameSlave = strcat(dir,strLeft,'_errAbsSlave','_',type);
