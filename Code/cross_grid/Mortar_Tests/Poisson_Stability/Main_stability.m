...
constraint_type = 'red';
% full: do not remove end-point lagrange multipliers
% red: remove endpoint multipliers
...
for type = ["DUAL","P0"]
   ...
   switch type
      case 'DUAL'
         ...
         leg = 'Dual';
      case 'STANDARD'
         brokenL2_STAND = zeros(nGrids,1);
         brokenH1_STAND = zeros(nGrids,1);
         leg = 'Standard';
      case 'P0'
   ...
   for mCount = 1:nGrids
      ...
      switch type
         ...
         case {'DUAL','CONF','STANDARD'}
            E = D\M;
            % handle end points
            if strcmp(constraint_type,'red')
               D(3,:) = D(3,:) + D(1,:);
               D(end,:) = D(end,:) + D(2,:);
               M(3,:) = M(3,:) + M(1,:);
               M(end,:) = M(end,:) + M(2,:);
      ...
      switch type
         case {'DUAL','STANDARD'}
            if strcmp(constraint_type,'full')
      ...
      switch type
         ...
         case 'STANDARD'
            brokenL2_STAND(mCount) = bL2;
            brokenH1_STAND(mCount) = bH1;
         case 'P0'
   ...
   switch type
      case {'DUAL','STANDARD','CONF','UNBIASED'}
         if strcmp(constraint_type,'full')
            [x,id] = sort(slaveMesh.coordinates(mortar.nodesSlave(1:end),1));
         else
...
legend show
exportgraphics(figure(1), strcat('Plots/instab_sides','.pdf'), 'Resolution', 300); % High resolution
