...
for type = ["DUAL","P0","CONF","UNBIASED"]
   ...
   for mCount = 1:nGrids
      ...
      if strcmp(type,'CONF')
         getMeshPois2D('Mesh_conv/domain_bottom.geo','bottom',NelX);
      else
   ...
   figure(1)
   plot(x,mult(id),'s-','LineWidth',1,'DisplayName',type)
...
nexttile
loglog(h, brokenL2_RBF, 'b-o', 'DisplayName', 'DUAL - Broken L2 Norm') % RBF in blu
hold on
loglog(h, brokenL2_P0, 'r-s', 'DisplayName', 'P0 - Broken L2 Norm') % P0 in rosso
loglog(h, brokenL2_CONF, 'k-s', 'DisplayName', 'CONFORMING - Broken L2 Norm') % P0 in rosso
loglog(h, h.^2 * (brokenL2_RBF(1)/h(1)^2), 'k-.', 'DisplayName', 'h^2') % Riferimento
legend show
xlabel('h')
ylabel('Error')
title('Broken L2 Norm')
