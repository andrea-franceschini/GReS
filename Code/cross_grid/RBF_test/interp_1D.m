...
nInt = 2; % number of interpolation points over each element (excluding nodes)
tol = 1e-5;

%% CASE 1 - interpolate basis function on a single element
a = -1;
b = 0;
pts = [a linspace(a+0.01,b-0.01,nInt) b];
N = @(x) 1/(b-a)*x - a/(b-a);
vals= N(pts);
...
plot(pts, vals, 'k-o', 'LineWidth',1)
hold on
plot(sample_c1,vals2_c1, 'r*')
