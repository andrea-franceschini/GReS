classdef Mortar2D < handle
   ...
   methods
      ...
      function H = computeStabilizationMatrix(obj,Em,Es)
         ...
         for n = obj.nodesSlave'
            ...
            if sum(edgeID)<2
               ...
               eS = find(edgeID); % slave cell ID in contact
               l1 = getLength(obj,eS(1));
               l2 = getLength(obj,eS(2));
               for e = eS'
                  % add stiffness contribution
                  Kslave = Kslave + obj.computeSlaveStabStiff(e,Es);
               end
               ...
               % cells    
               K = (0.25*l1*l2)*(2*inv(Kslave)); % 2 by 2 diagonal matrix
               Hloc = [K -K; -K K];
      ...
      function [HS,HM] = computeConfCrossGridMat(obj)
      ...
      function [D,M,varargout] = computeMortarConstant(obj,nGP,nInt)
         ...
         for j = 1:obj.nElSlave
            ...
            for jm = master_elems'
               ...
               % gather entries of D and M for stabilization contribution
            end
         end
         D = D(:,obj.nodesSlave);
         M = M(:, obj.nodesMaster);
      ...
      function [mf2c,sf2c] = buildFace2CellMap(obj)
         ...
         listCell = (1:obj.mshSlave.nSurfaces)';
         for i = 1:obj.nElSlave
            idMat = ismember(obj.mshSlave.surfaces(listCell,:),obj.slaveTopol(i,:));
            idMat = sum(idMat,2);
            id = find(idMat==2);
         ...
         listCell = (1:obj.mshMaster.nSurfaces)';
         for i = 1:obj.nElMaster
            idMat = ismember(obj.mshMaster.surfaces(listCell,:),obj.masterTopol(i,:));
            idMat = sum(idMat,2);
            id = find(idMat==2);
            assert(isscalar(id),'Invalid connectivity between face and cells');
            mf2c(i) = listCell(id);
      ...
      function K = computeSlaveStabStiff(obj,el,E)
         c = obj.f2cSlave(el);
         A = obj.mshSlave.surfArea(c);
         % size of the bounding box
         x = obj.mshSlave.coordinates(obj.mshSlave.surfaces(c,:),1);
         y = obj.mshSlave.coordinates(obj.mshSlave.surfaces(c,:),2);
         lx = abs(max(x) - min(x));
         ly = abs(max(y) - min(y));
         Ecell = E(c);
      ...
      function K = computeMasterStabStiff(obj,elM,elS,E)
         ...
         % size of the bounding box
         x = obj.mshMaster.coordinates(obj.mshMaster.surfaces(c,:),1);
         y = obj.mshMaster.coordinates(obj.mshMaster.surfaces(c,:),2);
         lx = abs(max(x) - min(x));
         ly = abs(max(y) - min(y));
         Ecell = E(c);
         K = diag((Ecell*A)./([lx^2;ly^2]));
      ...
      function getAreaMapMatrix(obj)
         % matrix computing the area shared between each pair of master and
         % slave elements (useful to weight the stiffness contribution in
         % the stabilization parameter)
         iVec = zeros(nnz(obj.elemConnectivity),1);
         jVec = iVec; 
         aVec = iVec;
         k = 0;
         for i = 1:obj.nElMaster
