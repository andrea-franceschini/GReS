classdef Mortar2D < handle
   ...
   methods
      ...
      function H = computeStabilizationMatrix(obj,KM,KS,D,M)
         ...
         for n = obj.nodesSlave'
            ...
            if sum(edgeID)<2
               ...
               nS = unique(obj.slaveTopol(eS,:));
               % get master elements in contact
               eM = [find(obj.elemConnectivity(:,eS(1)));find(obj.elemConnectivity(:,eS(2)))];
               % list of nodes in contact
               nM = unique(obj.masterTopol(eM,:));
               nSid = ismember(obj.nodesSlave,nS);
               nMid = ismember(obj.nodesMaster,nM);
               % 
               C = [-M(eS,nMid) D(eS,nSid)];
               % swap rows and change sign
               C = flipud(C);
               C(2,:) = -C(2,:);
               Kloc = diag([1./KM(nM);1./KS(nS)]);
               Hloc = C*(Kloc*C');
               H(eS,eS) = H(eS,eS)+Hloc;
            end
         end
      end
