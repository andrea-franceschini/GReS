classdef SinglePhaseFlow < SinglePhysics
  ...
  methods (Access = public)
    ...
    %   hf2Cell = repelem(ncell.lastCell+1,nrep(ncell.lastCell+1));
    %   L = obj.faces.faceCentroid(ncell.faces,:) - obj.mesh.cellCentroid(hf2Cell,:);
    %   % sgn = 2*(hf2Cell == obj.faces.faceNeighbors(ncell.face)) - 1;
    %   sgn(1:6)=1;
    %   N = bsxfun(@times,sgn,obj.faces.faceNormal(ncell.faces,:)')';
    %   KMat = zeros(obj.mesh.nCellTag,9);
    %   for i=1:obj.mesh.nCellTag
    %     KMat(i,:) = obj.material.getMaterial(i).PorousRock.getPermVector();
    %   end
    ...
    %   c = [1, 2, 3, 1, 2, 3, 1, 2, 3];
    %   nrep = diff(obj.faces.mapF2E);
    %   hf2Cell = repelem(ncell.volu.gtCell+1,nrep(ncell.volu.gtCell+1));
    %   L = obj.faces.faceCentroid(ncell.surf.face,:) - obj.mesh.cellCentroid(hf2Cell,:);
    %   % sgn = 2*(hf2Cell == obj.faces.faceNeighbors(ncell.face)) - 1;
    %   sgn(1:6)=-1;
    %   N = bsxfun(@times,sgn,obj.faces.faceNormal(ncell.surf.face,:)')';
    %   KMat = zeros(obj.mesh.nCellTag,9);
    %   for i=1:obj.mesh.nCellTag
    %     KMat(i,:) = obj.material.getMaterial(i).PorousRock.getPermVector();
    %   end
