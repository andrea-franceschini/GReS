...
%             PLoc = (vol/20)*(ones(obj.preP.nNodesElem(1)) + eye(obj.preP.nNodesElem(1)));
          case 12 % Hexa
    ...
    function computeFlowRHSGravContribute(obj)
      obj.rhsConst = zeros(obj.mesh.nNodes,1);
...
%               fLoc = (N'*permMat(:,3))*volSign*vol*gamma;
          ...
          %
          dof = obj.mesh.cells(el,1:obj.mesh.cellNumVerts(el));
          obj.rhsConst(dof) = obj.rhsConst(dof) + fLoc;
        end
      end
    end
    
    function computeFlowRHS(obj,statek,stateTmp)
      obj.rhs = obj.rhsConst;
      obj.rhs = obj.rhs + obj.K2*statek.pressure;
    end
