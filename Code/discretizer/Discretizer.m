classdef Discretizer < handle
    ...
    methods (Access = public)
        ...
        function computeFlowStiffAndCapMatFEM_Test(obj) %provisional method exploiting dof manager workflow
            ...
            for i = 1:nSub
                if any(strcmp(obj.dofm.subDomains(i).physics,'Flow'))
                    ...
                    for ii = 1:nSubPh
                        for jj = k:nSubPh
                            if strcmp(obj.blockJ(ii,jj).physic,'Flow')
            ...
            for i = 1:length(obj.dofm.subDomains)
                if any(strcmp(obj.dofm.subDomains(i).physics,'Flow'))
                    ...
                    for ii = 1:nSubPh
                        for jj = 1:nSubPh
                            if strcmp(obj.blockJ(ii,jj).physic,'Flow')
                                obj.blockJ(ii,jj).P = obj.blockJ(ii,jj).P + sparse(1:nSubCells,1:nSubCells,PVal,nSubCells,nSubCells);
                            end
                        end
                    end
                end
        ...
        function computePoroMat(obj,state,dt)
            % this method compute stiffness matrix for mechanics (independently from time
            % marching scheme)
            nSubPh = length(obj.dofm.subList);
            nSub = length(obj.dofm.subDomains);
            ...
            for i = 1:nSub
                if any(strcmp(obj.dofm.subDomains(i).physics,'Poro'))
                    ...
                    for ii = 1:nSubPh
                        for jj = k:nSubPh
                            if strcmp(obj.blockJ(ii,jj).physic,'Poro')
                                %find pairs of row,col indices belonging to ii,jj
                                %block of Jacobian
                                tmp = intersect(find(phVeci == ii),find(phVecj == jj));
                                obj.blockJ(ii,jj).K = obj.blockJ(ii,jj).K + sparse(iiVec(tmp),jjVec(tmp),KVec(tmp), ...
                                    obj.dofm.numDof(ii),obj.dofm.numDof(jj));
                                %consider a transpose option for the following line
                                if ii ~= jj
                                    obj.blockJ(jj,ii).K = (obj.blockJ(ii,jj).K)';
                                end
                            end
                        end
                        k = k+1;  %loop only trough upper triangular
        ...
        function computeBlockJacobian(obj,dt,varargin)
            % Loop trough the block structure of Jacobian and compute the
            % leading blocks from elementary blocks (e.g theta*H + P/dt)
            % this method applies theta marching scheme to the block Jacobian

            nBlock = size(obj.blockJ,1);
            for i=1:nBlock^2
                [rw, ~] = ind2sub(i, nBlock);
                switch obj.blockJ(i).physic
                    case 'Flow'
                        obj.blockJ(i).block = obj.simParams.theta*obj.blockJ(i).H + obj.blockJ(i).P/dt;
                    case 'Richards'
                        if obj.simParams.isNewtonNLSolver()
                            % Compute the additional terms of the Jacobian
                            [JNewt] = computeNewtPartOfJacobian(obj,dt,varargin{1}, ...
                                varargin{2},varargin{3},varargin{4},varargin{5});
                            obj.J = obj.J + JNewt;
                    ...
                    case 'Poro'
                        obj.blockJ(i).block = obj.simParams.theta*obj.blockJ(i).K;
                    case 'Biot'
                        if strcmp(obj.blockRhs(rw).physic,'Poro')
                            obj.blockJ(i).block = - obj.simParams.theta*obj.blockJ(i).Q;
                        elseif strcmp(obj.blockRhs(rw).physic,'Flow')
                            obj.blockJ(i).block = obj.blockJ(i).Q/dt;
                        end
                end
            end
        end

        function computeBlockRhs(obj,statek,stateTmp,dt)
            % compute Rhs for each block (avoid definition of single
            % methods)
            mu = obj.material.getFluid().getDynViscosity();
            lw = 1/mu;
            theta = obj.simParams.theta;
            gamma = obj.material.getFluid().getFluidSpecWeight();
            for i = 1:length(obj.blockRhs)
                switch obj.blockRhs(i).physic
                    case 'Flow'
                        for j = 1:length(obj.blockRhs)
                            switch obj.blockJ(i,j).physic
                                case 'Flow'
                                    dofs = obj.dofm.getLocDoF(j);
                                    obj.blockRhs(i).block = obj.blockRhs(i).block + ...
                                        theta*obj.blockJ(i,j).H*stateTmp.pressure(dofs) + ...
                                        (1-theta)*obj.blockJ(i,j).H*statek.pressure(dofs) + ...
                                        (obj.blockJ(i,j).P/dt)*(stateTmp.pressure(dofs) - statek.pressure(dofs));
                                case 'Biot'
                                    dofs = obj.dofm.getLocDoF(j);
                                    obj.blockRhs(i).block = obj.blockRhs(i).block +...
                                        obj.blockJ(i,j).block*(stateTmp.dispCurr(dofs) - ...
                                        stateTmp.dispConv(dofs));
                            end
                        end
                        if gamma > 0
        ...
        function computePoroRhs(obj,stateTmp)
            ...
            for i = 1:nRhs
                if strcmp(obj.blockRhs(i).physic,'Poro')
                    for j = 1:nRhs
                        if strcmp(obj.blockJ(i,j).physics,'Poro')
                            dofs = obj.dofm.getLocDoF(j);
                            obj.blockRhs(i).block = obj.blockRhs(i).block + ...
                                obj.blockJ(i,j).block*stateTmp.dispCurr(dofs) + ...
                                ((1-theta)/theta)*obj.blockJ(i,j).block*stateTmp.dispConv(dofs);
        ...
        function computePoroCoupled_Test(obj,state,dt)
            ...
            for i = 1:nSub
                if any(strcmp(obj.dofm.subDomains(i).physics,'Poro'))
                    ...
                    for ii = 1:nSubPh
                        for jj = k:nSubPh
                            if strcmp(obj.blockJ(ii,jj).physics,'Poro')
                                %find pairs of row,col indices belonging to ii,jj
                                %block of Jacobian
                                tmp = intersect(find(phVeci == ii),find(phVecj == jj));
        ...
        function computeBiotMat(obj)
            if isFEMBased(obj.model, 'Flow')
                obj.computeBiotMat_FEM_FEM_Test();
            elseif isFVTPFABased(obj.model,'Flow')
                obj.computeBiotMat_FEM_FV_Test
            end
        end

        function computeBiotMat_FEM_FEM_Test(obj)
            ...
            for i = 1:nSub
                if obj.dofm.subDomains(i).coupling
                    ...
                    for ii = 1:nSubPh
                        for jj = 1:nSubPh
                            if strcmp(obj.dofm.subPhysics(ii),'Poro') && strcmp(obj.blockJ(ii,jj).physic,'Biot')
                                %find pairs of row,col indices belonging to ii,jj
                                tmp = intersect(find(phVeci == ii),find(phVecj == jj));
                                Qtmp = sparse(iivec(tmp),jjvec(tmp),Qvec(tmp),obj.dofm.numDof(ii),obj.dofm.numDof(jj));
                                obj.blockJ(ii,jj).Q = obj.blockJ(ii,jj).Q + Qtmp;
                                obj.blockJ(jj,ii).Q = obj.blockJ(jj,ii).Q + Qtmp';
                            end
                        end
                    end
                end
        ...
        function computeBiotMat_FEM_FV_Test(obj)
            %loop search inside Jacobian block matrix
            nSubPh = size(obj.blockJ,1);
            nSub = length(obj.dofm.subDomains);
            % Loop trough elements of blockJ matrix to find pairs PORO-FLOW
        ...
        function computeBiotRhs(obj,stateTmp,statek)
            ...
            for i = 1:nRhs
                if strcmp(obj.blockRhs(i).physic,'Poro')
                    for j = 1:nRhs
                        if strcmp(obj.blockJ(i,j).physics,'Biot')
                            dofs = obj.dofm.getLocDoF(j);
                            obj.blockRhs(i).block = obj.blockRhs(i).block + ...
                                obj.blockJ(i,j).block*stateTmp.pressure(dofs) + ...
                                (1/theta-1)*(obj.blockJ(i,j).block*statek.pressure(dofs));
        ...
        function out = isLinear(obj,modStr)
            %check linearity of specified model
            % only check on Poromechanics is done up to now
            out = true;
            for i = 1:obj.mesh.nCellTag
                if ~isa(obj.material.getMaterial(i).ConstLaw,'Elastic')
                    out = false;
                end
            end
        end
    ...
    methods(Access = private)
        function setDiscretizer(obj,symmod,params,dofManager,grid,mat,data)
            ...
            %
            obj.initializeBlockJacobianAndRhs();
            obj.extractModels();
        ...
        function initializeBlockJacobianAndRhs(obj)
            ...
            dim = length(obj.dofm.subList);
            obj.blockJ = repmat(struct('block',[],'subID',[],'physic',[]),dim);
            ...
            for i = 1:dim
                ...
                for j=1:dim
                    ...
                    tmpPh = obj.dofm.subPhysics([i j]);
                    if any(ismember(tmpPh,'Poro')) && any(ismember(tmpPh, 'Flow'))
                        obj.blockJ(i,j).physic = 'Biot';
                    elseif all(ismember(tmpPh,'Poro'))
                        obj.blockJ(i,j).physic = 'Poro';
                    elseif all(ismember(tmpPh, 'Flow'))
                        obj.blockJ(i,j).physic = 'Flow';
                    end
                    % leading block of final jacobian
                    obj.blockJ(i,j).block = sparse(obj.dofm.numDof(i),obj.dofm.numDof(j));
                    % elementary blocks
                    switch obj.blockJ(i,j).physic
                        case 'Flow'
                        ...
                        case 'Poro'
                            obj.blockJ(i,j).K = sparse(obj.dofm.numDof(i),obj.dofm.numDof(j));
                        case 'Biot'
                            obj.blockJ(i,j).Q = sparse(obj.dofm.numDof(i),obj.dofm.numDof(j));
                    end
                end
            end
        end
        ...
        function extractModels(obj)
            n = length(obj.blockRhs)^2;
            mod = [];
            for i = 1:n
                mod = [mod, convertCharsToStrings(obj.blockJ(i).physic)];
            end
            obj.mods = unique(mod);
        end

        function computeSystBlocks(obj, ph, state, dt)
            % unique function for calling computation of local blocks
            for i = length(obj.mods)
                switch ph
                    case 'Poro'
                        obj.computePoroMat(state, dt);
                    case 'Flow'
                        obj.computeSPFMatrices();
                    case 'Biot'
                        obj.computeBiotMat();
                end
            end
