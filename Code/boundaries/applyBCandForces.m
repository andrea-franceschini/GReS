function applyBCandForces(model, grid, bound, material, t, syst, state)
  ...
  %Now coupled and uncoupled formulations are managed with lot of ifs
  %maybe a more "structured" way to impose BCs should be considered
  maxVal = max(abs(syst.J), [], 'all');
  %
  nDofPoro = grid.topology.nDim*grid.topology.nNodes;
  keys = bound.db.keys;
  %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% COUPLED  %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
if isPoromechanics(model) && isFlow(model)
     for i = 1 : length(keys)
       if strcmp(bound.getCond(keys{i}),'NodeBC')
          ...
          elseif strcmp(bound.getPhysics(keys{i}),'Flow')
            if strcmp(bound.getType(keys{i}), 'Neu')  % Apply Neumann conditions,if any
            syst.rhs(nDofPoro + bound.getDofs(keys{i})) = syst.rhs(nDofPoro + bound.getDofs(keys{i})) - bound.getVals(keys{i}, t);
            elseif strcmp(bound.getType(keys{i}), 'Dir')  % Apply Dirichlet conditions
            nrows = size(syst.J,1);
            syst.J(nrows*(nDofPoro + bound.getDofs(keys{i})-1) + nDofPoro + bound.getDofs(keys{i})) = maxVal*1.e10;
            syst.rhs(nDofPoro + bound.getDofs(keys{i})) = 0;
            end
          end
    ...
    %           entitiesForce = entitiesInfl*q;
              ...
              elseif strcmp(bound.getPhysics(keys{i}),'Flow')  
                ...
                q = bound.getVals(keys{i}, t);
    %           entitiesForce = entitiesInfl*q;
                syst.rhs(nDofPoro + loadedEnts) = syst.rhs(nDofPoro + loadedEnts) - entitiesInfl*q;               
              end
