classdef NonLinearSolver < handle
    ...
    methods (Access = public)
        ...
        function [simStat] = NonLinearLoop(obj)
            ...
            % compute poromechanics and couple blocks only once if the model is
            % linear
            %
            %
            % Loop over time
            while obj.t < obj.simParameters.tMax
                ...
                % end
                linSyst.computePoroMat(obj.stateTmp, delta_t)
                linSyst.computeBiotMat();
                linSyst.computeBlockJacobian(delta_t);
                linSyst.computeBlockRhs(obj.statek, obj.stateTmp, delta_t)
                ...
                while ((rhsNorm > tolWeigh) && (obj.iter < obj.simParameters.itMaxNR) ...
                    ...
                    % Compute new Jacobian and update rhs
                    linSyst.computePoroMat(obj.stateTmp, delta_t)
                    linSyst.computeBlockJacobian(delta_t);
                    linSyst.computeBlockRhs(obj.statek, obj.stateTmp, delta_t);
