classdef entityField
  ...
  methods
    ...
    function map = getEntityMapping(targetEntity,sourceEntity,mesh,varargin)
      % return an matrix performing geometric interpolation from a
      % sourceEntity to a targetEntitiy.
      % mesh (grid object handling all geometric information on the
      % entities)
      % since thisi is an interpolation operator, rows sum to one
