...
%  nPhyNames = 1;
%  el_PhyName = linspace(1,1,nTotElems);
nPhyNames = 2;
el_PhyName = cellTag;
one = linspace(1,1,nTotElems);
 ...
 fprintf(fid,'%g\n',nPhyNames);
 fprintf(fid,'3 1 "reservoir 1"\n');
 fprintf(fid,'3 2 "reservoir 2"\n');
...
%   end
  ...
  for m = 1:nTotElems
     fprintf(fid,'%g %g %g %g %g %g %g %g %g\n',m,codeEl,elem_flag(m),el_PhyName(m),cellTag(m),elemTopol(m,:));
  end
end
...
%   end
  ...
  for m = 1:nTotElems
     fprintf(fid,'%g %g %g %g %g %g %g %g %g %g %g %g %g\n',m,codeEl,elem_flag(m),el_PhyName(m),one(m),elemTopol(m,:));
  end
end
