...
elas = mat.getMaterial('hypoel');

sigma_sup = -50;   %MPa
% Calculating vertical stresses distribution due to material self-weight
weight = zeros(nTotNode,1);
...
for el = 1:nElem
    ...
    z = max_z - z;
    gamma = 2200/(10^6);
    weight(el) = -gamma*9.8*z+sigma_sup;  %MPa
end
...
K = assemblyKGauss(nTotNode, elemMAT, mat, hexa, weight);
autoval = eigs(K);
%spy(K)
% Reading time
AssemblyK_time = toc;
