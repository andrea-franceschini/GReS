...
// cross-platform modifications by ChatGPT, 2025-10-29
...
void read_polygons_MATLAB(const mxArray *prhs, Paths &poly) 
{
    ...
    for (mwIndex i = 0; i < num_contours; i++){
        ...
        int nrhs, const mxArray *prhs[])
{
    ...
    {   
        ...
        if (nrhs >= 3)
        {
            ...
            miterType = static_cast<char>(tolower(static_cast<unsigned char>(miterTypePtr[0])));
            mxFree(miterTypePtr);
            if (nrhs >= 4)
            {
    ...
    else if (nrhs >= 3)
    {
        ...
        if (mxIsStruct(prhs[1]))
        {
            ...
            switch (ct){
                case 0: CT=ctDifference; break;
                case 1: CT=ctIntersection; break;
                case 2: CT=ctXor; break;
                case 3: CT=ctUnion; break;
                default: mexErrMsgTxt("Third input must be 0, 1, 2, or 3.");
            }
            ...
            if (nrhs >= 4)
            {
                ...
                int sft = mxGetScalar(prhs[3]);
                switch (sft){
                    case 0: SFT=pftEvenOdd; break;
                    case 1: SFT=pftNonZero; break;
                    case 2: SFT=pftPositive; break;
                    case 3: SFT=pftNegative; break;
                    default: mexErrMsgTxt("Fourth input must be 0, 1, 2, or 3.");
                }
            }
            else
                SFT=pftEvenOdd;
            ...
            if (nrhs >= 5)
            {
                ...
                int cft = mxGetScalar(prhs[4]);
                switch (cft){
                    case 0: CFT=pftEvenOdd; break;
                    case 1: CFT=pftNonZero; break;
                    case 2: CFT=pftPositive; break;
                    case 3: CFT=pftNegative; break;
                    default: mexErrMsgTxt("Fifth input must be 0, 1, 2, or 3.");
                }
            }
            else
                CFT=pftEvenOdd;
