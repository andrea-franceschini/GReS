classdef NodeBC < handle
    ...
    methods (Access = public)
     ...
     function NodeBoundary(obj,varargin)
        % Memory allocation
        obj.node_ind = zeros(sum(obj.numID),1);
        i = 1;
        % Generating node_ind 
        ...
        for col = 1 : obj.dofmax
            for row = 1 : max(obj.numID)
                if obj.ID(row,col) ~= 0
                obj.node_ind(i) = obj.dofmax * (obj.ID(row,col)-1) + col;
                i = i + 1;
                end
            end
        end
        ...
        obj.node_value = zeros(sum(obj.numID),1);
        i = 1;
        % Generating node_value
        while i <= sum(obj.numID)
        for row = 1 : max(obj.numID)
            for col = 1 : obj.dofmax
                if obj.value(row,col) ~= 0
                obj.node_value(i,1) = obj.value(row,col);
                end
            end
        end
        i = i + 1;
