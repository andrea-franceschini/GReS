...
F = -10; % vertical force
applyTerzaghiIC(domain.state,mat,grid,F);
...
Terzaghi_analytical(grid, mat, abs(F),domain.outstate.timeList,output_dir)
...
if true
  ...
  nodesU = find(grid.coordinates(:,1)+grid.coordinates(:,2)==0);
  [~,ind] = sort(grid.coordinates(nodesU,3));
  nodesU = nodesU(ind);

  centers = grid.cells.center;
  ...
  if strcmp(flowscheme,"FEM")
    ...
    nodesP = find(centers(:,1) + centers(:,2) < 0.51);
    [~,ind] = sort(centers(nodesP,3));
    nodesP = nodesP(ind);
  end
  ...
  if strcmp(flowscheme,"FVTPFA")
    ptsY = centers(nodesP,3);
  else
    ptsY = grid.coordinates(nodesP,3);
  end
  ...
  figure('Position', [100, 100, 700, 700])
  hold on
  plotObj1 = plot(-dispplot/H,grid.coordinates(nodesU,3)/H,'k.', 'LineWidth', 1, 'MarkerSize', 15);
