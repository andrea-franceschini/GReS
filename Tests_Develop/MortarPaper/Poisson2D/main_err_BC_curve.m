...
command = "python Mesh/scripts/domain_bot_curve.py "  + fnameBot...
  + " " + num2str(5) + " " + num2str(3) + " " + elem_type;
system(command);

fnameTop = strcat('top_err','_',elem_type);
command = "python Mesh/scripts/domain_top_curve.py "  + fnameTop...
...
if strcmp(sol_scheme, 'SP')
  ...
  K = [Kmm, zeros(length(dofM),length(dofS)), KmIm, zeros(length(dofM),length(dofIs)), zeros(length(dofM),nmult);
    zeros(length(dofS),length(dofM)), Kss, zeros(length(dofS),length(dofIm)), KsIs, zeros(length(dofS),nmult) ;
    ...
    zeros(nmult, length(dofM)), zeros(nmult, length(dofS)), -M, D,  zeros(nmult,nmult)];
...
% slave interface after removing multiplier dofs
nodesLatSlave = unique(slaveMesh.edges(slaveMesh.edgeTag == 2,:));
...
plotFunction(masterMesh,'OUT_errMaster_curve',abs(u_master - u_anal_master));
c = slaveMesh.coordinates;
u_anal_slave = u_anal(c(:,1),c(:,2));
plotFunction(slaveMesh,'OUT_errSlave_curve',abs(u_slave - u_anal_slave));
