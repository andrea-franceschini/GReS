...
k = 1.00e-5; %[m/s] hydraulic conductivity
...
cellsP = arrayfun(@(alphan) bsxfun(@(x,t) ((cos(alphan)*(cos(alphan*x/a)-cos(alphan)))/(1-2*eta*(cos(alphan))^2))*exp(-alphan^2*t),x',t),alphan,'UniformOutput',false);
...
figure(2)
semilogx(dimless_time/time_factor, norm_p' ,'-')
