...
grainMesh = Mesh();
grainMesh.importMesh('pores.vtk');
...
nodesTop = find(abs(grainMesh.coordinates(:,3)-max(grainMesh.coordinates,[],'all'))<tol);
nodesBot = find(abs(grainMesh.coordinates(:,3)-min(grainMesh.coordinates,[],'all'))<tol);
...
printUtils = OutState(model,grainMesh,'outTime.dat','writeVtk',true,'folderName','OUT_p0_fewPores');
