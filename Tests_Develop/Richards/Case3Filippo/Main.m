...
%% -------------------------- SET THE PHYSICS -------------------------
if isSPF
    physics = "SinglePhaseFlow_FVTPFA";
else
    physics = "VariabSatFlow_FVTPFA";
end

model = ModelType(physics);
...
fileName = fullfile(input_dir,'Mesh','cube.msh');
% fileName = fullfile(input_dir,'Mesh','mesh_noB.msh');
...
% Set the input file name
if isSPF
    fileName = fullfile(input_dir,'materialsList2.dat');
else
...
cond(1).name = 'H_pressure';
cond(1).type = 'Spg';
cond(1).field = "X_0";
cond(1).times = 0.;
cond(1).values = 25.;
% 
% cond(2).name = 'Lpressure';
% cond(2).type = 'Spg';
% cond(2).field = "X_M";
% cond(2).times = 0;
% cond(2).values = 24.5;
...
% cond(1).values = 0.;
if isSPF
    fileName = setRichardsBC('Inputs',grid,cond,"SinglePhaseFlow");
else
    fileName = setRichardsBC('Inputs',grid,cond,"VariablySaturatedFlow");
end
...
wLev = 22.; % level of the water table
