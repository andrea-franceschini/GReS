...
%  fprintf("norm(S*v) = %2.4e \n",norm(S*v))
 ...
 v = [1e-4 1e-3 1e-2 1e-1 0.5 1 1.5 2 5 10 20 100 1e3 1e4 1e5];
  ...
  G = (1/h_s)*(B*invQ*B');
  %eAB = real(eig(full(G),full(K)));

  %eNew = real(eig(S,Q));

  infSup1 = sqrt(min(real(eig(invQ*S))));
  %infSup2 = sqrt(min(eAB));
  %infSup3 = sqrt(min(eNew));
...
figure
plot(log(1./NS),log(infSupStab1),'k-o','LineWidth',1.2)
xlabel("hS")
ylabel("infSup")
hold on
%plot(log(1./NS),log(infSupStab2),'b-s','LineWidth',1.2)
ylim([-1 0])
