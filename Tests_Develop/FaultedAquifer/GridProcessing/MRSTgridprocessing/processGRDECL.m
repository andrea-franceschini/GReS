...
function [G, P, B] = create_initial_grid(X, Y, Z, preserve_cpnodes)
   ...
   %  in view of mortar algorithm, unique must be applied only to nodes belonging to unfaulted faces.
   % 
...
% partially pinched faces (triangular faces)
f(f(:,1)==f(:,4), 1)=nan;
f(f(:,2)==f(:,3), 2)=nan;
...
% the artifical z coordinate separates the stacks. this allow use to
% process stacks separately, while keeping a e b as list of contiguous
% arrays.
...
if all(all (pa == pb))
   ...
   Corners    = reshape(pa', [], 1);
    ...
    % this follows by how PP and QQ where constructed to find the
    % logical array of intersection. 
    % adding size(points,1) appends new point indices to the grid
    % structure
    ...
    % this seems a safety measure to avoid introducing pinched nodes in the
    % new faces
...
% we walk from the top to the bottom of the cell stack
...
for i = 1 : size(za, 1) - 1
    ...
    % a_i)
     while any(zb(j,:) < za(i+1,:), 2)
       ...
       % this basically freeze the j index when exiting the b loop. 
       % at the next face of the a stack, we do not miss previous face of the
       % b stack 
